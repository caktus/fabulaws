...
logger = logging.getLogger('fabulaws.sftp')
...
class SFTP(object):
    ...
    def put(self, local_path, remote_path, use_sudo, mirror_local_mode, mode,
        ...
        if (local_is_path and mirror_local_mode) or (mode is not None):
            ...
            if lmode != rmode:
                if use_sudo:
                    self.ssh.sudo('chmod %o \"%s\"' % (lmode, remote_path))
                else:
                    self.ftp.chmod(remote_path, lmode)
        if use_sudo:
            self.ssh.sudo("mv \"%s\" \"%s\"" % (remote_path, target_path))
