...
import paramiko
from boto.ec2 import blockdevicemapping, elb
...
class EC2Instance(object):
    ...
    def __init__(
        ...
        if (
            (not self.ami or not self.user or not self.instance_type)
            and not instance_id
            and not instance
        ):
            raise Exception(
                "You must extend this class and define the ami, "
        ...
        if terminate or tags or placement:
            logger.warning(
                "The terminate, tags, and placement arguments "
    ...
    def _create_instances(
        self, instance_id=None, count=1, ami=None, placement=None, wait_ssh=True
    ):
        """
    ...
    def _wait_for_ssh(self, instance):
        ...
        while times < 120 / wait:
            try:
                ...
                ssh.connect(
                    getattr(
                        instance, getattr(env, "ec2_attr_for_ssh", "public_dns_name")
                    ),
                    allow_agent=False,
                    look_for_keys=False,
                    username=user,
                ...
                logger.debug(
                    "Error connecting ({0}); retrying in {1} " "seconds".format(e, wait)
                )
                times += 1
                time.sleep(wait)
        ssh.close()
    ...
    def _setup_context(self):
        ...
        if self.key_file:
            logger.debug(
                'Setting env.key_filename = "{0}"' "".format(self.key_file.name)
            )
        ...
        if self.user:
            ...
            env.user = self.user
        host_string = getattr(
            self.instance, getattr(env, "ec2_attr_for_ssh", "public_dns_name")
        )
        logger.debug('Setting env.host_string = "{0}"' "".format(host_string))
    ...
    def reset_authentication(self):
        ...
        if len(self._saved_contexts) > 0:
            raise ValueError(
                "reset_authentication() can only be called when "
    ...
    def add_to_elb(self, elb_name):
        ...
        try:
            ...
            logger.info(
                "Failed to call enable_availability_zones(). IGNORE this error if using a VPC!"
            )
    ...
    def elb_state(self, elb_name):
        ...
        """
        try:
            return self.elb_conn.describe_instance_health(elb_name, [self.instance.id])[
                0
            ].state
        except BotoServerError:
    ...
    def wait_for_elb_state(self, elb_name, state, max_wait=300):
        ...
        while curr_state != state:
            if waited > max_wait:
                raise Exception(
                    "Instance {inst} did not enter {state} state in "
                    " LB {elb} after waiting {secs} seconds"
                    "".format(
                        inst=self.instance.id, state=state, elb=elb_name, secs=max_wait
                    )
                )
            time.sleep(wait)
            waited += wait
    ...
    def create_image(self, replace_existing=False, name=None):
        ...
        while status != "available":
            logger.info(
                "Unexpected image status after pending: %s...waiting a bit longer...",
                status,
            )
            time.sleep(2)
            status = image.update()
    ...
    def create_copies(self, count, placement=None, recreate_image=True, **kwargs):
        ...
        instances = self._create_instances(
            count=count, ami=image.id, placement=placement, wait_ssh=False
        )
    ...
    def hostname(self):
        if self.instance:
            ...
            raise ValueError(
                "No instance has been created yet, or the "
