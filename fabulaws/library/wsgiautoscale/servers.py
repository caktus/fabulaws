...
class BaseInstance(FirewallMixin, UbuntuInstance):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.volume_info = [
            ("/dev/sdf", self.app_root, self.volume_size, self.volume_type, passwd)
        ]
    ...
    def setup_swap(self):
        ...
        if swap_mb > 0:
            if not files.exists(self.default_swap_file):
                ...
                print(
                    "swap file {0} already exists; skipping".format(
                        self.default_swap_file
                    )
                )
    ...
    def setup_sudoers(self):
        ...
        files.upload_template(
            sudoers_file, "/etc/sudoers.new", backup=False, use_sudo=True, mode=0o440
        )
    ...
    def setup(self):
        ...
        self.instance.modify_attribute(
            "blockDeviceMapping", ["%s=true" % v[0] for v in self.volume_info]
        )
...
class DbMixin(PostgresMixin):
    ...
    def setup(self):
        ...
        self.pg_allow_replication(
            "%s_repl" % env.database_user,
